# Pyclaude Feature Coverage Matrix
...
| Category | OMC Features | Pyclaude Implemented | Coverage |
|----------|-------------|-----------------|----------|
...
| OMC Agent | Pyclaude Status | Mechanism |
|-----------|-----------|-----------|
...
| OMC Skill | Pyclaude Status | Mechanism |
|-----------|-----------|-----------|
...
| deepinit | DONE (lightweight CLI successor) | `pyclaude agents-init [path]` (`pyclaude deepinit [path]` alias) |
...
| pyclaude-setup | DONE | ~/.codex/skills/pyclaude-setup/SKILL.md |
...
| SessionEnd | pyclaude postLaunch lifecycle phase | PARTIAL (post-exit cleanup) |

`*` FULL via terminal automation workaround (default-enabled in `v0.2.3` generated `.pyclaude/tmux-hook.json`), not native hook context injection.

### Infrastructure

| Component | OMC | Pyclaude Status |
|-----------|-----|-----------|
| CLI (setup) | DONE | DONE |
...
2. **Native context injection from hooks** - Not available in Codex hooks API. Workaround: tmux prompt injection (`pyclaude tmux-hook`) plus state files + AGENTS.md instructions (default-enabled in `v0.2.3` generated config).
...
4. **Session end** - No real-time event. Workaround: `pyclaude` wrapper detects Codex exit via blocking execSync and runs postLaunch cleanup (overlay strip, session archive, mode cancellation).
