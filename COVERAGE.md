...
| Category | OMC Features | Orbit Implemented | Coverage |
|----------|-------------|-----------------|----------|
...
| OMC Agent | Orbit Status | Mechanism |
|-----------|-----------|-----------|
...
| OMC Skill | Orbit Status | Mechanism |
|-----------|-----------|-----------|
...
| deepinit | DONE (lightweight CLI successor) | `orbit agents-init [path]` (`orbit deepinit [path]` alias) |
...
| orbit-setup | DONE | ~/.codex/skills/orbit-setup/SKILL.md |
...
| SessionEnd | orbit postLaunch lifecycle phase | PARTIAL (post-exit cleanup) |

`*` FULL via terminal automation workaround (default-enabled in `v0.2.3` generated `.orbit/tmux-hook.json`), not native hook context injection.

### Infrastructure

| Component | OMC | Orbit Status |
|-----------|-----|-----------|
| CLI (setup) | DONE | DONE |
...
2. **Native context injection from hooks** - Not available in Codex hooks API. Workaround: tmux prompt injection (`orbit tmux-hook`) plus state files + AGENTS.md instructions (default-enabled in `v0.2.3` generated config).
...
4. **Session end** - No real-time event. Workaround: `orbit` wrapper detects Codex exit via blocking execSync and runs postLaunch cleanup (overlay strip, session archive, mode cancellation).
