...
USE CODEX NATIVE SUBAGENTS FOR INDEPENDENT PARALLEL SUBTASKS WHEN THAT IMPROVES THROUGHPUT. THIS IS COMPLEMENTARY TO Orbit TEAM MODE.
<!-- END AUTONOMY DIRECTIVE -->

# Orbit - Intelligent Multi-Agent Orchestration
<!-- orbit:generated:agents-md -->

You are running with Orbit (Orbit), a coordination layer for Codex CLI.
...
- `<!-- Orbit:RUNTIME:START --> ... <!-- Orbit:RUNTIME:END -->`
- `<!-- Orbit:TEAM:WORKER:START --> ... <!-- Orbit:TEAM:WORKER:END -->`
</guidance_schema_contract>
...
<!-- Orbit:GUIDANCE:OPERATING:START -->
...
- Do not hardcode stale frontier-model overrides for Codex native child agents. If an explicit frontier override is necessary, use the current frontier default from `Orbit_DEFAULT_FRONTIER_MODEL` / the repo model contract (currently `gpt-5.4`), not older values such as `gpt-5.2`.
...
- Planning is complete only after both `.orbit/plans/prd-*.md` and `.orbit/plans/test-spec-*.md` exist.
...
Model precedence:
1. Explicit model in `Orbit_TEAM_WORKER_LAUNCH_ARGS`
2. Inherited leader `--model`
3. Low-complexity default model from `Orbit_DEFAULT_SPARK_MODEL` (legacy alias: `Orbit_SPARK_MODEL`)

Normalize model flags to one canonical `--model <value>` entry.
Do not guess frontier/spark defaults from model-family recency; use `Orbit_DEFAULT_FRONTIER_MODEL` and `Orbit_DEFAULT_SPARK_MODEL`.
</team_model_resolution>

<!-- Orbit:MODELS:START -->
<!-- Auto-generated by orbit setup -->
<!-- Orbit:MODELS:END -->
...
<!-- Orbit:GUIDANCE:VERIFYSEQ:START -->
...
Command Routing:
- When `USE_Orbit_EXPLORE_CMD` enables advisory routing, strongly prefer `orbit explore` as the default surface for simple read-only repository lookup tasks (files, symbols, patterns, relationships).
- For simple file/symbol lookups, use `orbit explore` FIRST before attempting full code analysis.
- Keep ambiguous, implementation-heavy, edit-heavy, or non-shell-only work on the normal Codex path.
- If `orbit explore` is unavailable or fails, gracefully fall back to the normal path.
- Let `orbit explore` keep direct inspection by default and use `orbit sparkshell` only as an adaptive backend for qualifying read-only shell-native tasks.
- For explicit tmux-pane / worker / leader / HUD inspection, prefer `orbit sparkshell --tmux-pane ...` when a larger-tail read or bounded summary is useful. Sparkshell pane mode is explicit opt-in, not always-on.

When to use what:
- If the task is a simple read-only file/symbol/pattern/relationship lookup -> use `orbit explore` first.
- If the task is a noisy read-only shell command, verification run, repo-wide search/listing, or tmux-pane summary -> use `orbit sparkshell`.
...
- If `orbit explore` or `orbit sparkshell` returns incomplete or ambiguous results -> retry with a narrower prompt/command, then fall back to the normal path.

Explore Usage:
- Use `orbit explore` as the default surface for simple read-only file, symbol, pattern, and relationship lookups.
- Keep `orbit explore` prompts narrow and concrete; prefer a single lookup goal or a small related cluster over broad multi-part investigation.
- Prefer `orbit explore --prompt ...` for quick one-off lookups and `orbit explore --prompt-file ...` for longer reusable briefs.
- Good explore examples: `orbit explore --prompt "which files define TeamPolicy"` and `orbit explore --prompt "find usages of buildExploreRoutingGuidance"`.
- Expect a shell-only, allowlisted, read-only path; do not rely on `orbit explore` for edits, tests, diagnostics, MCP/web access, or complex multi-command shell composition.
- If `orbit explore` cannot answer safely, stalls, or returns incomplete results, retry with a narrower prompt or fall back to the richer normal path.

Sparkshell Usage:
- Protect context budget by default: strongly prefer `orbit sparkshell` for noisy read-only and verification commands where full raw output is usually wasteful.
- Prefer `orbit sparkshell` for repository search/listing, bounded file reads, build/test/typecheck runs, and tmux-pane summarization.
- Good sparkshell examples: `orbit sparkshell -- rg -n "TeamPolicy" src`, `orbit sparkshell -- npm test`, and `orbit sparkshell --tmux-pane %12`.
- Treat `orbit sparkshell` as an augmenting layer, not a full shell replacement; use raw shell when exact stdout/stderr, shell composition, or low-level debugging fidelity is required.
...
- If `orbit sparkshell` returns incomplete, ambiguous, or `summary unavailable` output, immediately retry with a more precise command or the raw shell.
...
- Persist verdict JSON in `.orbit/state/{scope}/ralph-progress.json`.
...
<state_management>
Orbit persists runtime state under `.orbit/`:
- `.orbit/state/` — mode state
- `.orbit/notepad.md` — session notes
- `.orbit/project-memory.json` — cross-session memory
- `.orbit/plans/` — plans
- `.orbit/logs/` — logs
...
Run `orbit setup` to install all components. Run `orbit doctor` to verify installation.
