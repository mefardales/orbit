...
<Execution_Policy>
- Never use ad-hoc `tmux send-keys` as primary control; use `omx team api` CLI mutations
...
Example: `omx team 3:executor "analyze feature X and report flaws"`
...
2. **Monitor progress** via `omx team status <team-name>` and mailbox files
3. **Wait** for terminal task state (no pending/in-progress/failed tasks)
4. **Shutdown**: `omx team shutdown <team-name>`
5. **Verify cleanup**
...
- `omx team status <team-name>` -- reads task counts and worker health
- `omx team resume <team-name>` -- reconnects to active team
- `omx team shutdown <team-name>` -- graceful shutdown and cleanup

## Message Dispatch Policy

1. Use `omx team ...` runtime lifecycle commands
2. Use `omx team api ... --json` for mutations
...
| Issue | Solution |
|-------|----------|
| Stale panes from prior runs | Clean up with `omx team shutdown` |
| Trigger submit failures | Check worker state first via `omx team status` |
