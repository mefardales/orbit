...
from ..omx_utils.paths import get_omx_home as codex_home
...
def _read_omx_config_file(
    codex_home_override: Optional[str] = None,
) -> Optional[dict[str, Any]]:
    config_path = Path(codex_home_override or codex_home()) / ".omx-config.json"
...
def _read_models_block(
    ...
    config = _read_omx_config_file(codex_home_override)
    if not config:
        return None
...
def read_configured_env_overrides(
    ...
    """Read env overrides from .omx-config.json."""
    config = _read_omx_config_file(codex_home_override)
    if not config:
        return {}
    env_block = config.get("env")
...
def get_env_configured_main_default_model(
    ...
    return _normalize_configured_value(
        env.get(OMX_DEFAULT_FRONTIER_MODEL_ENV)
...
def get_env_configured_standard_default_model(
    ...
    if env is None:
        env = dict(os.environ)
    return _normalize_configured_value(
        env.get(OMX_DEFAULT_STANDARD_MODEL_ENV)
...
def get_env_configured_spark_default_model(
    ...
    if env is None:
        env = dict(os.environ)
    return (
        _normalize_configured_value(env.get(OMX_DEFAULT_SPARK_MODEL_ENV))
        or _normalize_configured_value(env.get(OMX_SPARK_MODEL_ENV))
