"""
Config.toml generator/merger for orbit.

Merges Orbit MCP server entries and feature flags into existing config.toml.
...
SHARED_MCP_REGISTRY_MARKER = "orbit Orbit Shared MCP Registry Sync"
SHARED_MCP_REGISTRY_END_MARKER = "# End orbit shared MCP registry sync"
ORBIT_AGENTS_MAX_THREADS = 6
ORBIT_AGENTS_MAX_DEPTH = 2
ORBIT_EXPLORE_ROUTING_DEFAULT = "1"
ORBIT_EXPLORE_CMD_ENV = "USE_ORBIT_EXPLORE_CMD"
ORBIT_TUI_STATUS_LINE = (
...
def _get_omx_top_level_lines(
    ...
    lines = [
        "# orbit top-level settings (must be before any [table])",
        f'notify = ["node", "{escaped_path}"]',
        'model_reasoning_effort = "high"',
        (
            'developer_instructions = "You have orbit installed. AGENTS.md is your '
...
def _strip_root_level_keys(config: str, keys: list[str]) -> str:
    ...
    if any(k in ORBIT_TOP_LEVEL_KEYS for k in keys):
        lines = [
            l
            for l in lines
            if l.strip()
...
def strip_omx_top_level_keys(config: str) -> str:
    """Remove any existing Orbit-managed top-level keys so we can re-insert them cleanly."""
    return _strip_root_level_keys(config, list(ORBIT_TOP_LEVEL_KEYS))
...
def _upsert_env_settings(config: str) -> str:
    ...
    if env_start < 0:
        base = config.rstrip()
        env_block = f'[env]\n{ORBIT_EXPLORE_CMD_ENV} = "{ORBIT_EXPLORE_ROUTING_DEFAULT}"\n'
        if not base:
            return env_block
        return f"{base}\n\n{env_block}"
    ...
    for i in range(env_start + 1, section_end):
        if re.match(rf"^\s*{ORBIT_EXPLORE_CMD_ENV}\s*=", lines[i]):
            explore_idx = i
            break
    ...
    if explore_idx < 0:
        lines.insert(
            section_end,
            f'{ORBIT_EXPLORE_CMD_ENV} = "{ORBIT_EXPLORE_ROUTING_DEFAULT}"',
        )
...
def _upsert_agents_settings(config: str) -> str:
    ...
    if agents_start < 0:
        base = config.rstrip()
        agents_block = (
            f"[agents]\nmax_threads = {ORBIT_AGENTS_MAX_THREADS}\n"
            f"max_depth = {ORBIT_AGENTS_MAX_DEPTH}\n"
        )
        if not base:
            return agents_block
        return f"{base}\n\n{agents_block}"
    ...
    if max_threads_idx < 0:
        lines.insert(section_end, f"max_threads = {ORBIT_AGENTS_MAX_THREADS}")
        section_end += 1
    if max_depth_idx < 0:
        lines.insert(section_end, f"max_depth = {ORBIT_AGENTS_MAX_DEPTH}")

    return "\n".join(lines)


def strip_omx_feature_flags(config: str) -> str:
    """Remove Orbit-managed feature flags from the [features] section."""
    lines = config.split("\n")
    features_start = -1
    for i, line in enumerate(lines):
        if re.match(r"^\s*\[features\]\s*$", line):
...
def strip_omx_env_settings(config: str) -> str:
    ...
    for i, line in enumerate(lines):
        if env_start < i < section_end:
            if re.match(rf"^\s*{ORBIT_EXPLORE_CMD_ENV}\s*=", line):
                continue
        filtered.append(line)
...
def _strip_orphaned_omx_sections(config: str) -> str:
    """Strip Orbit-managed table sections that exist outside the marker block."""
    lines = config.split("\n")
    result: list[str] = []
    ...
    while i < len(lines):
        ...
        if table_match:
            ...
            if is_omx_section:
                while result and (
                    result[-1].strip() == ""
                    or re.match(r"^#\s*(Orbit|orbit)", result[-1], re.IGNORECASE)
                ):
                    result.pop()
                i += 1
...
def _upsert_tui_status_line(config: str) -> tuple[str, bool]:
    ...
    merged_section = ["[tui]"] + preserved_key_lines + [ORBIT_TUI_STATUS_LINE]
...
def strip_existing_orbit_blocks(config: str) -> tuple[str, int]:
    """Strip Orbit configuration blocks. Returns (cleaned, removed_count)."""
    marker = "orbit Orbit Configuration"
    end_marker = "# End orbit"
    cleaned = config
    removed = 0
...
def _get_omx_tables_block(pkg_root: str, include_tui: bool = True) -> str:
    ...
    parts = [
        "",
        "# ============================================================",
        "# orbit Orbit Configuration",
        "# Managed by omx setup - manual edits preserved on next setup",
        "# ============================================================",
        "",
        "# Orbit State Management MCP Server",
        ...
        "",
        "# Orbit Project Memory MCP Server",
        ...
        "startup_timeout_sec = 5",
        "",
        "# Orbit Code Intelligence MCP Server (LSP diagnostics, AST search)",
        ...
        "",
        "# Orbit Trace MCP Server (agent flow timeline & statistics)",
        ...
        "startup_timeout_sec = 5",
        "",
        "# Orbit Team MCP Server (team job lifecycle: start, status, wait, cleanup)",
    ...
    if include_tui:
        parts.extend([
            "",
            "# Orbit TUI StatusLine (Codex CLI v0.101.0+)",
            "[tui]",
            ORBIT_TUI_STATUS_LINE,
            "",
        ])
...
def build_merged_config(
    ...
    """Merge Orbit config into existing config.toml."""
    if options is None:
    ...
    if "orbit Orbit Configuration" in existing:
        existing, _ = strip_existing_orbit_blocks(existing)
...
async def merge_config(
    ...
    """Merge Orbit config and write to disk."""
    if options is None:
        options = MergeOptions()
    ...
    if "orbit Orbit Configuration" in existing:
        _, removed = strip_existing_orbit_blocks(existing)
        if options.verbose and removed > 0:
            print("  Updating existing Orbit config block.")
