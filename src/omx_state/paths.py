"""State path resolution for oh-my-codex.
...
def get_base_state_dir(project_root: Optional[str] = None) -> str:
    """Return the base .omx/state directory for the given project root."""
...
def _read_current_session_id(project_root: Optional[str] = None) -> Optional[str]:
    ...
    env_val = os.environ.get("OMX_SESSION_ID", "").strip()
    if env_val and validate_session_id(env_val):
        return env_val

    root = project_root or os.getcwd()
    marker = Path(root) / ".omx" / "state" / "current-session.json"
    if marker.exists():
        try:
...
def resolve_state_scope(project_root: Optional[str] = None) -> ResolvedStateScope:
    ...
    if session_id:
        return ResolvedStateScope(
            ...
            source="env" if os.environ.get("OMX_SESSION_ID") else "file",
        )
    return ResolvedStateScope(
