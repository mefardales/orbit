...
def resolve_model() -> str:
    for var in ("OMX_SPARKSHELL_MODEL", "OMX_DEFAULT_SPARK_MODEL", "OMX_SPARK_MODEL"):
...
def resolve_fallback_model() -> str:
    for var in ("OMX_SPARKSHELL_FALLBACK_MODEL", "OMX_DEFAULT_FRONTIER_MODEL"):
        val = os.environ.get(var, "").strip()
        if val:
            return val
...
def read_summary_timeout_ms() -> int:
    raw = os.environ.get("OMX_SPARKSHELL_SUMMARY_TIMEOUT_MS", "")
    try:
        val = int(raw.strip())
